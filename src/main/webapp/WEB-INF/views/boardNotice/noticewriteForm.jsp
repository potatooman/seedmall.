...
<script>
	...
	$(function() {
		CKEDITOR.replace('ncontent', {//해당 이름으로 된 textarea에 에디터를 적용
			width : '100%',
			height : '400px',
			filebrowserUploadUrl : '<c:url value="noticeImg" />?${_csrf.parameterName}=${_csrf.token}'
		});
		$('#bf').submit(function() {
			var regExp = /^[0-9]{1,3}$/;
...
<div align="center" id="bbs" class="col-md-8 my-4" style="width: 1100px; margin: auto;">
	...
	<form name="bf" id="bf" role="form"
		action="/seedmall/boardNotice/noticeWrite?${_csrf.parameterName}=${_csrf.token}" method="POST"
		...
		<table class="table">
			...
			<tr>
				...
				<td style="width: 80%"><input type="text" name="name" id="name"
					class="form-control" readonly value="${loginUser.member.getMname()}">
					<input type="hidden" name="midx" id="midx"
					value="${loginUser.member.getMidx()}"></td>
			</tr>
			<tr>
				<td style="width: 8%">
					<b>글내용</b>
