...
<script>
	...
	function userEdit(vidx, i) {
		//alert(vidx+"//"+i)
		let enabled = $('#enabled' + i + " option:selected").val();
		//alert(enabled);
		$('#midx2').val(vidx);
		$('#enabled').val(enabled);
			...
			<tbody>
				<c:forEach var="user" items="${userArr}" varStatus="st">
					...
					<tr>
						<td>${user.midx}</td>
						<td>${user.email}</td>
						<td>${user.mname}<br> <c:if test="${user.auth eq 'ROLE_ADMIN'}">[관리자]</c:if>
						<c:if test="${user.auth eq 'ROLE_MEMBER'}">[${user.grade}]</c:if>
						</td>
						<td>${user.allHp}</td>
						<td>
						...
						<td>${user.mdate}</td>
						<td><c:if test="${user.auth eq 'ROLE_MEMBER'}">
								<select id="enabled${st.index}" name="enabled" style="padding: 1px;">
									<option value="1"
										<c:if test="${user.enabled eq 1}">selected</c:if>>일반회원</option>
									<option value="0" style="color: red;"
										<c:if test="${user.enabled eq 0}">selected</c:if>>정지회원</option>
								</select>
							</c:if>
