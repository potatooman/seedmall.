...
<script>
	...
	function myInfo() {
		...
		if(result){
			let name = "${loginUser.member.mname}";
			let hp1 = "${loginUser.member.mhp1}";
			let hp2 = "${loginUser.member.mhp2}";
			let hp3 = "${loginUser.member.mhp3}";
			let post = "${loginUser.member.mpost}";
			let addr1 = "${loginUser.member.maddr1}";
			let addr2 = "${loginUser.member.maddr2}";
	...
	function iamport(){
		...
		let uid = "order_${loginUser.member.midx}_"+nowDate.YYYYMMDDHHMMSS();
		let prodName = "시드몰 : ${orderArr[0].pname}";
		let total = ${total};
		let email = '${loginUser.member.email}' 
		let name = $('#omname').val(); 
